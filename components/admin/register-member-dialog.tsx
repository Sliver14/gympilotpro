  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="h-10 px-6 bg-primary hover:bg-[#cdb48b] text-black font-black text-[10px] gap-2 rounded-xl transition-all shadow-xl shadow-primary/10">
      ...
      <DialogContent className="bg-card border-border text-foreground rounded-[2.5rem] p-5 md:p-10 max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
        <DialogHeader className="space-y-4">
          <DialogTitle className="text-3xl font-black uppercase tracking-tighter">
            New Member <span className="text-primary">Enrollment</span>
          </DialogTitle>
          <DialogDescription className="text-muted-foreground font-medium text-[10px] leading-relaxed">
            Establishing a new profile in the gym. Default password: <span className="text-primary font-black">12345678</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  <SelectTrigger className="h-14 bg-background border-border rounded-xl focus:border-primary px-4 font-black text-[10px] flex-1">
                  ...
                  <SelectContent className="bg-card border-border text-foreground">
                    {[
                      ...
                      <SelectItem key={m.value} value={m.value} className="focus:bg-primary/10 focus:text-primary font-bold text-[10px]">
                        {m.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                >
                  <SelectTrigger className="h-14 bg-background border-border rounded-xl focus:border-primary px-4 font-black text-[10px] w-24">
                  ...
                  <SelectContent className="bg-card border-border text-foreground">
                    {(() => {
                      ...
                      return Array.from({ length: daysInMonth }, (_, i) => {
                        ...
                        return (
                          <SelectItem key={dayStr} value={dayStr} className="focus:bg-primary/10 focus:text-primary font-bold text-[10px]">
                            {dayNum}
                          </SelectItem>
                        )
                      })
            ...
            <div className="space-y-3">
              ...
              >
                <SelectTrigger id="gender" className="h-14 bg-background border-border rounded-xl focus:border-primary px-4 font-black text-[10px]">
                  <SelectValue placeholder="SELECT GENDER" />
                </SelectTrigger>
                <SelectContent className="bg-card border-border text-foreground">
                  {GENDER_OPTIONS.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value} className="focus:bg-primary/10 focus:text-primary font-bold text-[10px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div className="space-y-3">
              ...
              >
                <SelectTrigger id="membership" className="h-14 bg-background border-border rounded-xl focus:border-primary px-4 font-black text-[10px]">
                ...
                <SelectContent className="bg-card border-border text-foreground">
                  {PAYMENT_METHODS.map((method) => (
                    <SelectItem 
                      ...
                      className="focus:bg-primary/10 focus:text-primary font-bold text-[10px]"
                    >
          ...
          <div className="space-y-3">
            ...
            <Textarea
              ...
              rows={3}
              className="bg-background border-border rounded-2xl focus:border-primary p-4 md:p-6 font-bold text-xs"
            />
          </div>
