      ...
      <div className="relative z-10">
        ...
        {notes.length === 0 ? (
          <div className="text-center py-20 bg-card/50 rounded-3xl border border-dashed border-border">
            ...
            <p className="mt-4 text-sm font-bold text-muted-foreground">Progress notes pending trainer briefing.</p>
          </div>
        ) : (
            ...
            {notes.map((note) => (
              <div key={note.id} className="rounded-2xl bg-card/50 border border-border p-4 md:p-6 hover:border-[#daa857]/30 transition-all group">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge className="bg-[#daa857]/10 text-[#daa857] border-[#daa857]/20 text-[8px] font-black px-2">Elite Trainer</Badge>
                    </div>
                  ...
                  {note.weight && (
                    <div className="bg-background border border-[#daa857]/20 rounded-xl px-4 py-2 text-center group-hover:border-[#daa857] transition-colors">
                      <p className="text-xl font-black text-[#daa857] leading-none">{note.weight}</p>
                ...
                <div className="mt-6 p-4 rounded-xl bg-accent border border-border group-hover:border-border transition-colors">
                  <p className="whitespace-pre-wrap text-sm font-medium text-muted-foreground leading-relaxed">
                    "{note.note}"
                  </p>
                </div>
              </div>
